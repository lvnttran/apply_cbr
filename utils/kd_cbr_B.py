...
class KdCbrB(KdCbrBase):
    def __init__(self):
        KdCbrBase.__init__(self)
        # Load Excel file into a pandas ExcelFile object
        excel_file = r'excel/df_sheet_B_nna_nol_tp_dia.xlsx'
        pd.set_option('display.max_rows', None)
        self.df_sheetOT = pd.read_excel(excel_file)

        kmeans_model_path = r'src/models/kmean_B.joblib'
        ...
        # Example usage:
        train_excel_path = r'excel/df_sheet_B_nna_nol_tp_dia_k_train.xlsx'
        test_excel_path = r'excel/df_sheet_B_nna_nol_tp_dia_k_test.xlsx'
        self.df_sheetB_80_dict, _ = self.split_list_to_dict(train_excel_path, test_excel_path)
        df_sheetB_80_dict_modified = self.df_to_filtered_dict(self.df_sheetB_80_dict, self.column_names)
        X = self.dict_to_np_array(df_sheetB_80_dict_modified, self.column_names)
        # Train the KDTree
        self.kdtree = KDTree(X)
        self.weight_set = [1, 1, 1, 1, 1, 1, 3]
